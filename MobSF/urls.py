...
from MobSF.views.home import Recent_scans
from MobSF.views.login.login import LoginView, VerifyView, VerifyUserNameView
from MobSF.views.login.repwd import RePwdView, XadminRePwdView, XadminAddUserView
...
urlpatterns = [
    ...
    # url(r'^index', home.index, name='home'),
    ...
    url(r'^recent_scans$', Recent_scans.as_view(), name='recent'),
    ...
    url(r'^verifyusername$', VerifyUserNameView.as_view()),  # 验证用户名是否存在
    url(r'^repwd$', RePwdView.as_view()),
    url(r'^adminMobSF/user/add/$', XadminAddUserView.as_view()),
    url(r'admin', xadmin.site.urls),
    url('adminMobSF/user/(?P<user_id>\d+)/password', XadminRePwdView.as_view()),  # xadmin修改密码接口
