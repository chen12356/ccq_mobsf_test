...
from MobSF.views.login.utils import get_user_by_token, verify_jwt
    ...
    def post(self, request):
        ...
        if user.is_superuser:
            ...
            for i in range(len(db_obj_item)):
                db_obj.append(RecentScansDB.objects.filter(MD5=db_obj_item[i].get("MD5")).last().to_dict())
            db_obj = db_obj[::-1]
        else:
            db_obj = RecentScansDB.objects.filter(USER_ID=user.id).order_by('-TIMESTAMP').values()
        entries = []
                ...
                if scan.exists():
                    if user.is_superuser:
                        ...
                        app_upload_dir = os.path.join(settings.UPLD_DIR, md5_hash)
                         # Delete Upload Dir Contents
                        KEYS = [MODEL_K % ("RecentScansDB", md5_hash), MODEL_K % ("StaticAnalyzerAndroid", md5_hash),
                            MODEL_K % ("StaticAnalyzerIOS", md5_hash), MODEL_K % ("StaticAnalyzerWindows", md5_hash)]
                        for key in KEYS:
                             RDS_DB5.delete(key)
                        # Delete Upload Dir Contents
                        app_upload_dir = os.path.join(settings.UPLD_DIR, md5_hash)
                        ...
                        # 普通用户只能删掉自己上传的app
                        RecentScansDB.objects.filter(MD5=md5_hash, USER_ID=user.id).delete()
                        # StaticAnalyzerAndroid.objects.filter(MD5=md5_hash).delete()
                        StaticAnalyzerIOS.objects.filter(MD5=md5_hash).delete()
                        StaticAnalyzerWindows.objects.filter(MD5=md5_hash).delete()
            data = {'deleted': 'yes'}
