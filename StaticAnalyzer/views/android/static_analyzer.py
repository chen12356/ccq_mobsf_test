...
from StaticAnalyzer.views.url_email_analysis import main as handle_data, RDS_DB5
    ...
    try:
        ...
        if (
            ...
            if typ == 'apk':
                ...
                key = MODEL_K % ('StaticAnalyzerAndroid', app_dic['md5'])
                db_entry = StaticAnalyzerAndroid.objects.filter(
                    MD5=app_dic['md5'])
                if db_entry.exists() and rescan == '0':
                    logger.info('SEARCH REDIS:%s' % key)
                    con = RDS_DB5.get(key)  # 获取缓存数据
                    context = pickle.loads(con) if con else None  # 获取缓存数据
                    if not context:  # 如果没有数据,查询数据库
                        logger.info('SEARCH MYSQL, INSERT REDIS:%s' % key)
                        context = get_context_from_db_entry(db_entry)
                        RDS_DB5.set(key, pickle.dumps(context))  # 写入缓存中
                else:
                    ...
                    man_an_dic = manifest_analysis(
                        ...
                        man_data_dic)
                    bin_an_buff = []
                    code_dict = elf_analysis(app_dic['app_dir'])
                    bin_an_buff += code_dict[0]
                    so_list = code_dict[1]
                    bin_an_buff += res_analysis(app_dic['app_dir'])
                    # 调用so_analysis函数获取SDK
                    res_so_data=so_analysis(so_list)
                    cert_dic = cert_info(
                        app_dic['app_dir'],
                        app_dic['app_file'])
                    ...
                    if string_res:
                        ...
                        app_dic['strings'] = []
                    # 解析urls与emails
                    code_an_dic['urls'] = handle_data(types='urls', datas=code_an_dic['urls'])
                    code_an_dic['emails'] = handle_data(types='emails', datas=code_an_dic['emails'])
